...
export default function PreparePage({ versionId }: PreparePageProps) {
  ...
  const fetchPrepareData = async () => {
    const response = await fetch(`/api/assessment/prepare?versionId=${versionId}`, {
      method: 'GET',
      headers: {
  ...
  const editPrepareDataViewOrder = async (companyProcessId: number, toViewOrder: number) => {
    const response = await fetch(
  ...
  const { data, error, isLoading } = useQuery('prepareData', fetchPrepareData, {
