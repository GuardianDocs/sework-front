...
const NavbarLink: React.FC<NavbarLinkProps> = ({ href, children }) => (
  <Link href={href} className="font-semibold text-white cursor-pointer text-ml hover:text-green-200">
    {children}
  </Link>
);
...
const NavbarLinks = () => {
  const links = [
    ...
    { href: '/assessment/start', label: '위험성평가 시작' },
...
const UserStatus = () => {
  ...
  return companyName ? (
    <>
      <span className="font-semibold text-slate-700">{companyName}님</span>
    ...
    <div className="bg-blue-400">
      <nav className="flex items-center justify-between p-5 ">
        <div className="flex items-center h-8">
          <Link href="/" className="text-xl font-semibold text-white cursor-pointer hover:text-green-200">
            I-RAS
          </Link>
        </div>
